...
func TestImportCFDICreatesPurchaseAndAutoRegistersSupplier(t *testing.T) {
	...
	if line.LinkStatus != domain.LinkPending {
		t.Fatalf("link status = %q, want PENDIENTE for a brand new supplier product", line.LinkStatus)
	}
...
func TestImportCFDIRejectsDocumentWithoutFiscalStamp(t *testing.T) {
	service, _, _ := newTestService()
	unstamped := []byte(`<cfdi:Comprobante xmlns:cfdi="http://www.sat.gob.mx/cfd/4" Version="4.0" Fecha="2026-01-01T00:00:00" SubTotal="1" Moneda="MXN" Total="1">
		<cfdi:Emisor Rfc="ABC010101AA1"/></cfdi:Comprobante>`)
...
func TestImportCFDIReusesExistingResourceLinkForKnownSKU(t *testing.T) {
	...
	if _, err := service.LinkSupplierProductToResource(t.Context(), supplierProductID, 555); err != nil {
		t.Fatalf("link supplier product: %v", err)
	}
	...
	if second.Lines[0].LinkStatus != domain.LinkLinked {
		t.Fatalf("link status = %q, want VINCULADO because the SKU is already known", second.Lines[0].LinkStatus)
	}
