...
func TestCatalogListPassesPublicQueryToCore(t *testing.T) {
	...
	h := NewRouter(reader, nil, nil, nil)
...
func TestCatalogListMapsNonemptyPageThroughHTTP(t *testing.T) {
	...
	r := httptest.NewRecorder()
	NewRouter(reader, nil, nil, nil).ServeHTTP(r, httptest.NewRequest(http.MethodGet, "/v1/catalog/UNIDAD", nil))
	body := r.Body.Bytes()
	var page struct {
		Records []struct {
			ID       string                     `json:"id"`
...
func TestCatalogListDefersUnknownKindToCore(t *testing.T) {
	...
	r := httptest.NewRecorder()
	NewRouter(reader, nil, nil, nil).ServeHTTP(r, httptest.NewRequest(http.MethodGet, "/v1/catalog/FUTURE", nil))
	var response errorResponse
...
func TestCatalogListSanitizesUnavailableReaderAndMappingErrors(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			r := httptest.NewRecorder()
			NewRouter(tc.reader, nil, nil, nil).ServeHTTP(r, httptest.NewRequest(http.MethodGet, "/v1/catalog/UNIDAD", nil))
			var response errorResponse
...
func TestCatalogListRequiresGETAndRejectsUnsupportedExtraSegment(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.method+tc.path, func(t *testing.T) {
			r := httptest.NewRecorder()
			NewRouter(reader, nil, nil, nil).ServeHTTP(r, httptest.NewRequest(tc.method, tc.path, nil))
...
func TestCatalogDetailPassesExactPositiveKeysToCore(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.id, func(t *testing.T) {
			reader.keys = nil
			r := httptest.NewRecorder()
			NewRouter(reader, nil, nil, nil).ServeHTTP(r, httptest.NewRequest(http.MethodGet, "/v1/catalog/UNIDAD/"+tc.id, nil))
...
func TestCatalogDetailRejectsMalformedIDWithoutCallingCore(t *testing.T) {
	...
	for _, id := range []string{"0", "00", "01", "-1", "+1", " 1", "1 ", "1.0", "1e1", "9223372036854775808"} {
		t.Run(id, func(t *testing.T) {
			...
			NewRouter(reader, nil, nil, nil).ServeHTTP(r, req)
			var response errorResponse
...
func TestCatalogDetailDefersUnknownKindToCore(t *testing.T) {
	...
	r := httptest.NewRecorder()
	NewRouter(reader, nil, nil, nil).ServeHTTP(r, httptest.NewRequest(http.MethodGet, "/v1/catalog/FUTURE/1", nil))
	var response errorResponse
	if err := json.NewDecoder(r.Body).Decode(&response); err != nil || r.Code != http.StatusBadRequest || response.Error != "invalid request" {
		t.Fatalf("status/error = %d/%q, decode error = %v", r.Code, response.Error, err)
	}
...
func TestCatalogDetailMapsRecordAndSanitizesFailures(t *testing.T) {
	...
	h := NewRouter(&listCatalogReader{get: func(context.Context, resourcecore.CatalogKey) (resourcecore.CatalogRecord, error) { return record, nil }}, nil, nil, nil)
	r := httptest.NewRecorder()
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			r := httptest.NewRecorder()
			NewRouter(tc.reader, nil, nil, nil).ServeHTTP(r, httptest.NewRequest(http.MethodGet, "/v1/catalog/UNIDAD/1", nil))
			var response errorResponse
			if err := json.NewDecoder(r.Body).Decode(&response); err != nil || r.Code != tc.status || response.Error != tc.message {
				t.Fatalf("status/error = %d/%q, decode error = %v", r.Code, response.Error, err)
			}
