...
func TestCatalogDescriptorsNormalizesNilSlice(t *testing.T) {
	h := NewRouter(catalogReaderFunc(func(context.Context) ([]resourcecore.CatalogDescriptor, error) { return nil, nil }), nil)
	r := httptest.NewRecorder()
...
func TestCatalogDescriptorsRequiresGET(t *testing.T) {
	called := false
	h := NewRouter(catalogReaderFunc(func(context.Context) ([]resourcecore.CatalogDescriptor, error) { called = true; return nil, nil }), nil)
	r := httptest.NewRecorder()
